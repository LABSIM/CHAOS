...
function configure() {
	...
	# name
	GAIA_TARGET_PRETTY_NAME="SilverLining"
...
function push_download_op_to_cache() {
	...
	echo "wget wget http://cdn.sundog-soft.com/${GAIA_TARGET_PRETTY_NAME}-SDK-Evaluation.tgz" >> exec.sh
	echo "mv ${GAIA_TARGET_PRETTY_NAME}-SDK-Evaluation.tgz ${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.tar.gz" >> exec.sh
...
function push_build_op_to_cache() {
	...
	# correcct the Makefile
	sed -i 's/DEBUG_FLAGS = -O3 -c/DEBUG_FLAGS = -O3 -c -fpermissive/g' Makefile
...
function push_install_op_to_cache() {
	...
	echo "cp -rvf SampleCode ${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}" >> exec.sh
