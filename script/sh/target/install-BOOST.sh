...
function push_download_op_to_cache() {
	...
	echo "wget https://dl.bintray.com/boostorg/release/${GAIA_TARGET_VERSION}/source/${GAIA_TARGET_LC_NAME}_${GAIA_TARGET_VERSION//./_}.tar.gz" >> exec.sh
	echo "mv ${GAIA_TARGET_LC_NAME}_${GAIA_TARGET_VERSION//./_}.tar.gz ${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.tar.gz" >> exec.sh
...
function push_extract_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "tar -xzvf ${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.tar.gz" >> exec.sh
...
###~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~##
#
# PUSH_BOOTSTRAP_OP_TO_CACHE FUNCTION
#
###~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~##

function push_bootstrap_op_to_cache() {

	# print
	echo -ne "  + Bootstrap de ${GAIA_TARGET_PRETTY_NAME} ${GAIA_TARGET_VERSION}..."
...
###~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~##
#
# PUSH_BUILD_AND_INSTALL_OP_TO_CACHE FUNCTION
#
###~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~##

function push_build_and_install_op_to_cache() {

	# print
	echo -ne "  + Build & installation de ${GAIA_TARGET_PRETTY_NAME} ${GAIA_TARGET_VERSION}..."

	# the op
	echo "#!/bin/bash" > exec.sh
	echo "./b2 --prefix=${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION} toolset=gcc address-model=64 variant=release threading=multi link=shared runtime-link=shared -j${GAIA_PARALLEL_BUILD_JOB_COUNT} install" >> exec.sh
	echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
...
print_header
	...
	# == bootstrap ==
	push_bootstrap_op_to_cache
	pop_cache

	# == build & install ==
	push_build_and_install_op_to_cache
	pop_cache
