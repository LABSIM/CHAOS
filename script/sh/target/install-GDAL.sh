...
function push_download_op_to_cache() {
	...
	echo "wget http://download.osgeo.org/${GAIA_TARGET_LC_NAME}/${GAIA_TARGET_VERSION}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.tar.xz" >> exec.sh
...
function push_configure_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "./configure --prefix=${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION} --enable-lto --with-xerces=${GAIA_THIRD_PARTY_HOME}/xerces-${GAIA_THIRD_PARTY_XERCES_VERSION}" >> exec.sh
	echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
