...
function push_download_op_to_cache() {
	...
	if [ "${GAIA_FOUND_AVAILABLE_INTERNET_CONNECTIVITY}" = true ]; then
		echo "wget http://dist.schmorp.de/${GAIA_TARGET_LC_NAME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_MAJOR}.${GAIA_TARGET_MINOR}.tar.gz" >> exec.sh
