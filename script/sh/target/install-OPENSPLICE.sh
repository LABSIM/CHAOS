...
function push_clone_op_to_cache() {
	...
	echo "git clone https://github.com/ADLINK-IST/${GAIA_TARGET_LC_NAME}.git" >> exec.sh
...
function push_configure_op_to_cache() {
	...
	echo "#!/bin/bash" > exec.sh
	echo "sed -i 's/= -std=c99 -D_GNU_SOURCE/= -fcommon -std=c99 -D_GNU_SOURCE/g' setup/x86_64.linux-default.mak" >> exec.sh
	#echo "sed -i 's/= -D_GNU_SOURCE -DOSPL_LINUX/= -fcommon -D_GNU_SOURCE -DOSPL_LINUX/g' setup/x86_64.linux-default.mak" >> exec.sh
