...
echo "#!/bin/bash" > exec.sh
echo "cmake -DCMAKE_INSTALL_PREFIX=$GAIA_THIRD_PARTY_HOME/openscenegraph-$OPENSCENEGRAPH_INSTALL_TARGET_VERSION -DCMAKE_BUILD_TYPE=Release -DOSG_USE_FLOAT_BOUNDINGBOX=OFF -DOSG_USE_FLOAT_BOUNDINGSPHERE=OFF -DOSG_USE_FLOAT_MATRIX=OFF -DOSG_USE_FLOAT_PLANE=OFF -DOSG_USE_DEPRECATED_API=OFF -DOSG_USE_REF_PTR_SAFE_DEREFERENCE=ON .." >> exec.sh
echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
