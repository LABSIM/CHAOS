...
function configure() {
	...
	# name
	GAIA_TARGET_PRETTY_NAME="OpenSceneGraph"
...
function push_clone_op_to_cache() {
	...
	echo "git clone https://github.com/${GAIA_TARGET_LC_NAME}/${GAIA_TARGET_PRETTY_NAME}.git" >> exec.sh
...
function push_checkout_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "git checkout tags/${GAIA_TARGET_PRETTY_NAME}-${GAIA_TARGET_VERSION}" >> exec.sh
	echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
...
function push_configure_op_to_cache() {
	...
	# cmake exist
	echo "#!/bin/bash" > exec.sh
	echo "cmake -DCMAKE_INSTALL_PREFIX=${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION} -DCMAKE_BUILD_TYPE=Release -DOSG_USE_FLOAT_BOUNDINGBOX=OFF -DOSG_USE_FLOAT_BOUNDINGSPHERE=OFF -DOSG_USE_FLOAT_MATRIX=OFF -DOSG_USE_FLOAT_PLANE=OFF -DOSG_USE_DEPRECATED_API=OFF -DOSG_USE_REF_PTR_SAFE_DEREFERENCE=ON .." >> exec.sh
	echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
