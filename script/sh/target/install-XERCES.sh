...
function configure() {
	...
	# name
	GAIA_TARGET_PRETTY_NAME="Xerces-C++"
	GAIA_TARGET_UC_NAME="$(echo Xerces | tr '[:lower:]' '[:upper:]')"
	GAIA_TARGET_LC_NAME="$(echo Xerces | tr '[:upper:]' '[:lower:]')"
...
function push_download_op_to_cache() {
	...
	echo "wget http://mirrors.ircam.fr/pub/apache//${GAIA_TARGET_LC_NAME}/c/${GAIA_TARGET_MAJOR}/sources/${GAIA_TARGET_LC_NAME}-c-${GAIA_TARGET_VERSION}.tar.gz" >> exec.sh
	echo "mv ${GAIA_TARGET_LC_NAME}-c-${GAIA_TARGET_VERSION}.tar.gz ${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.tar.gz" >> exec.sh
