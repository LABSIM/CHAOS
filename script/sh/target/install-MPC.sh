...
function push_configure_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "../configure --prefix=${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION} --with-gmp=${GAIA_THIRD_PARTY_HOME}/gmp-${GAIA_THIRD_PARTY_GMP_VERSION} --with-mpfr=${GAIA_THIRD_PARTY_HOME}/mpfr-${GAIA_THIRD_PARTY_MPFR_VERSION}" >> exec.sh
