...
function push_download_op_to_cache() {
	...
	if [ "${GAIA_FOUND_AVAILABLE_INTERNET_CONNECTIVITY}" = true ]; then
		echo "wget http://isl.gforge.inria.fr/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_MAJOR}.${GAIA_TARGET_MINOR}.tar.xz" >> exec.sh
...
function push_configure_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "./configure --prefix=${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION} --with-gmp-prefix=${GAIA_THIRD_PARTY_HOME}/gmp-${GAIA_THIRD_PARTY_GMP_VERSION}" >> exec.sh
