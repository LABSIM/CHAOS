...
function push_configure_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "./configure --prefix=${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION} --with-gmp=${GAIA_THIRD_PARTY_HOME}/gmp-${GAIA_THIRD_PARTY_GMP_VERSION}" >> exec.sh
	echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
