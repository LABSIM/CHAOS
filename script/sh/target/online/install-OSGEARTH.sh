...
PROTOBUF_INSTALL_TARGET_MIN_VER=6
PROTOBUF_INSTALL_TARGET_PATCH_VER=2
...
echo "#!/bin/bash" > exec.sh
echo "cmake -DCMAKE_INSTALL_PREFIX=$GAIA_THIRD_PARTY_HOME/osgearth-$OSGEARTH_INSTALL_TARGET_VERSION -DCMAKE_BUILD_TYPE=Release -DGDAL_DIR=$GAIA_THIRD_PARTY_HOME/gdal-$GDAL_INSTALL_TARGET_VERSION -DGEOS_DIR=$GAIA_THIRD_PARTY_HOME/geos-$GEOS_INSTALL_TARGET_VERSION .." >> exec.sh
echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
