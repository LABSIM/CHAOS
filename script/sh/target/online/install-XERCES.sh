...
# variables
CURRENT_DIR=$PWD
BUILD_DIR=/tmp/GAIA/$(whoami)/build/xercesc
...
echo "wget http://mirrors.ircam.fr/pub/apache//xerces/c/$XERCESC_INSTALL_TARGET_MAJ_VER/sources/xerces-c-$XERCESC_INSTALL_TARGET_VERSION.tar.gz" >> exec.sh
...
echo "#!/bin/bash" > exec.sh
echo "../configure --prefix=$GAIA_THIRD_PARTY_HOME/xercesc-$XERCESC_INSTALL_TARGET_VERSION" >> exec.sh
echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
