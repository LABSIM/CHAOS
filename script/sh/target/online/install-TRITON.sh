...
echo "#!/bin/bash" > exec.sh
echo "wget http://cdn.sundog-soft.com/latest/install-triton-fullsource.run" >> exec.sh
echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
echo -ne "\tTelechargement de Triton $TRITON_INSTALL_TARGET_VERSION ... "
chmod u+x exec.sh
...
echo "#!/bin/bash" > exec.sh
echo "sh install-triton-fullsource.run" >> exec.sh
echo "read -p \"Appuyez sur [Entree] pour continuer...\"" >> exec.sh
echo -ne "\tRun de Triton $TRITON_INSTALL_TARGET_VERSION ... "
chmod u+x exec.sh
gnome-terminal --working-directory $PWD --title="LABSIM - Triton $TRITON_INSTALL_TARGET_VERSION" --command "./exec.sh" --window
sleep 0.2
PID=$(pgrep exec.sh)
