...
function push_download_op_to_cache() {
	...
	echo "wget https://github.com/${GAIA_TARGET_LC_NAME}/${GAIA_TARGET_LC_NAME}/archive/v${GAIA_TARGET_VERSION}.tar.gz" >> exec.sh
	echo "mv v${GAIA_TARGET_VERSION}.tar.gz ${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.tar.gz" >> exec.sh
...
function push_configure_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "make configure" >> exec.sh
	echo "./configure --prefix=${GAIA_THIRD_PARTY_HOME}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION} --with-openssl=${GAIA_THIRD_PARTY_HOME}/openssl-${GAIA_THIRD_PARTY_OPENSSL_VERSION} --with-perl=${GAIA_THIRD_PARTY_HOME}/perl-${GAIA_THIRD_PARTY_PERL_VERSION}" >> exec.sh
