...
function push_copy_op_to_cache() {
	...
	echo "cp --verbose ${GAIA_OFFLINE_DIR}/${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.zip ." >> exec.sh
...
function push_extract_op_to_cache() {
	...
	# the op
	echo "#!/bin/bash" > exec.sh
	echo "unzip ${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.zip" >> exec.sh
	echo "rm ${GAIA_TARGET_LC_NAME}-${GAIA_TARGET_VERSION}.zip" >> exec.sh
